...
export function renderPopover(element: Element, step: DriveStep) {
    ...
    onDriverClick(
        ...
        (e) => {
            ...
            const onNextClick =
                step.popover?.onNextClick || getConfig("onNextClick");
            const onPrevClick =
                step.popover?.onPrevClick || getConfig("onPrevClick");
            const onCloseClick =
...
function calculateTopForLeftRight(
    ...
    if (alignment === "start") {
        return Math.max(
            Math.min(
                // CUSTOM CODE
                elementDimensions.top + offsetY - popoverPadding,
                window.innerHeight -
...
function calculateLeftForTopBottom(
    ...
    if (alignment === "start") {
        return Math.max(
            Math.min(
                // CUSTOM CODE
                elementDimensions.left + offsetX - popoverPadding,
                window.innerWidth -
...
export function repositionPopover(element: Element, step: DriveStep) {
    ...
    // CUSTOM CODE
    const offsetX = step.popover?.offsetX || 0;
    const offsetY = step.popover?.offsetY || 0;
    ...
    // CUSTOM CODE
    const topValue =
        elementDimensions.top + offsetY - popoverDimensions!.height;
    ...
    // CUSTOM CODE
    const leftValue =
        elementDimensions.left + offsetX - popoverDimensions!.width;
    ...
    const rightValue =
        window.innerWidth -
        offsetX -
        (elementDimensions.right + popoverDimensions!.width);
    ...
    if (requiredSide === "over") {
        ...
        // CUSTOM CODE
        popover.wrapper.style.left = `${leftToSet + offsetX}px`;
        popover.wrapper.style.right = `auto`;
        // CUSTOM CODE
        popover.wrapper.style.top = `${topToSet + offsetY}px`;
...
function renderPopoverArrow(
    ...
    // CUSTOM CODE
    const elementLeft = elementDimensions.left + offsetX;

    const popoverHeight = popoverDimensions.height;
    const windowHeight = window.innerHeight;
    // CUSTOM CODE
    const elementTop = elementDimensions.top + offsetY;
    ...
    if (side === "top") {
        ...
        if (elementTop + elementHeight <= 0) {
            ...
            // CUSTOM CODE
            alignment === "center" &&
            elementTop + elementHeight - popoverHeight / 2 > 0
        ) {
            arrowSide = "left";
    ...
    if (!arrowSide) {
        ...
        popoverArrow.classList.add(
            `driver-popover-arrow-align-${arrowAlignment}`
        );
        ...
        const isElementPartiallyInViewPort =
            elementRect.left + offsetX - stagePadding < window.innerWidth &&
            elementRect.right - offsetX + stagePadding > 0 &&
            elementRect.top + offsetY - stagePadding < window.innerHeight &&
            elementRect.bottom - offsetY + stagePadding > 0;
        if (side === "bottom" && isElementPartiallyInViewPort) {
            const isArrowWithinElementBounds =
                arrowRect.x > elementRect.x &&
                arrowRect.x + arrowRect.width <
                    elementRect.x + elementRect.width;

            if (!isArrowWithinElementBounds) {
                popoverArrow.classList.remove(
                    `driver-popover-arrow-align-${arrowAlignment}`
                );
                popoverArrow.classList.add(`driver-popover-arrow-none`);
                // reduce the top position by the padding
                popover.wrapper.style.transform = `translateY(-${
                    stagePadding / 2
                }px)`;
            } else {
